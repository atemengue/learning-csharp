...
namespace Chapter04.Exercices.Exercices04
{
  ...
  public class TextCounter
  {
    ...
    {
      Console.WriteLine($"ReadLine stop word file: {stopWordPath}");
      _stopWords = new HashSet<string>(File.ReadAllLines(stopWordPath));
    }

    public IEnumerable<Tuple<string, int>> Process(string text, int maximunWords)
    {
      var words = Regex.Split(text.ToLower(), @"\s+")
        .Where(text => !_stopWords.Contains(text))
        .GroupBy(t => t)
  ...
  class Program
  {
    public static void Main()
    {
      ...
      do
      {
        ...
        {
          Console.WriteLine($"Processing {bookTextLineCount:N0} lines ({bookText.Length:NO} characters)..");
